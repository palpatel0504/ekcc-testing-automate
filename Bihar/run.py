...
def select_state_and_district(page) -> None:
    """Select State = Bihar and District = Aurangabad (robust)."""

    # ── STATE ─────────────────────────────
    log_step("Selecting State: Bihar")
    ...
    state_input.fill("Bihar")

    state_option = page.get_by_role(
        "option",
        name=re.compile(r"Bihar", re.IGNORECASE)
    ).first

    state_option.wait_for(state="visible", timeout=10000)
    state_option.click()

    log_step("Selected State: Bihar")
    ...
    # ── DISTRICT ─────────────────────────
    log_step("Selecting District: Aurangabad")
    ...
    district_input.fill("Aurangabad")

    district_option = page.get_by_role(
        "option",
        name=re.compile(r"Aurangabad", re.IGNORECASE)
    ).first

    district_option.wait_for(state="visible", timeout=10000)
    district_option.click()

    log_step("Selected District: Aurangabad")
    pause(page, 2000)


def select_bank_name(page) -> None:
    """Select 'Bihar Gramin Bank' from the Bank Name dropdown."""
    log_step("Selecting bank name: Bihar Gramin Bank")
    ...
    for target in bank_name_targets:
        try:
            if target.count() and target.first.is_visible():
                ...
                option = page.get_by_role(
                    "option",
                    name=re.compile(r"Bihar Gramin Bank", re.IGNORECASE)
                ).first
...
def select_branch(page) -> None:
    """Select 'AURANGABAD Branch BRANCH' from the Branch dropdown."""
    branch_name = "AURANGABAD Branch BRANCH"
...
def select_district_araria(page) -> None:
    """Select District = Araria."""
    log_step("Selecting district: Araria")
    select_combobox_option_by_id(page, "districtMasterId", "Araria", "district")
    pause(page, 1000)


def select_subdistrict_araria(page) -> None:
    """Select Sub-District = Araria."""
    log_step("Selecting sub-district: Araria")
    field = page.locator("#subDistrictMasterId")
    field.wait_for(state="visible", timeout=10000)
    field.scroll_into_view_if_needed()
    pause(page, 1000)
    select_combobox_option_by_id(page, "subDistrictMasterId", "Araria", "sub-district")

def select_village_araria(page) -> None:
    """Select Village = Araria (50)."""
    log_step("Selecting village: Araria (50)")
...
def fill_bihar_land_numbers(page) -> tuple[str, str]:
    """Fill Khata No, Khasra No + Owner Names (chip inputs)."""

    khata_no = str(random.choice([
        123, 321, 444, 777, 888, 654, 3456, 23453, 224,
        567, 22454, 674, 2221, 536, 235, 257, 2346778, 909
    ]))

    khasra_no = str(random.choice([
        11, 22, 33, 44, 55, 66, 77, 88, 99,
        101, 202, 303, 404, 505, 606
    ]))

    log_step(f"Filling Bihar land → Khata: {khata_no}, Khasra: {khasra_no}")

    # ── Khata No ─────────────────────────
    khata_field = page.locator("#surveyNo")
    khata_field.wait_for(state="visible", timeout=10000)
    khata_field.fill(khata_no)
    pause(page, 500)

    # ── Khasra No ────────────────────────
    khasra_field = page.locator("#khasaraNo")
    khasra_field.wait_for(state="visible", timeout=10000)
    khasra_field.fill(khasra_no)
    pause(page, 500)

    # ── Owner Name (Land Records) ────────
    add_token_input(
        page,
        [
            page.get_by_label(re.compile(r"Owner.*Land Records", re.IGNORECASE)),
            page.locator("xpath=//label[contains(., 'Land Records')]/following::input[1]"),
        ],
        "pal",
    ...
    add_token_input(
        page,
        [
            page.get_by_label(re.compile(r"Aadhaar", re.IGNORECASE)),
            page.locator("xpath=//label[contains(., 'Aadhaar')]/following::input[1]"),
        ],
        "pal r patel",
        "Owner Name as per Aadhaar"
    )

    log_step("Bihar land numbers + owner names filled.")

    return khata_no, khasra_no
...
def fill_land_details_form(page) -> None:
    ...
    # ✅ FIXED ENCODED BLOCK
    log_step("Selecting Encumbered: No")

    encumbered_no_label = page.locator(
        "xpath=//div[label[contains(.,'Encumbered')]]//label[.//span[text()='No']]"
    ).first

    encumbered_no_label.wait_for(state="visible", timeout=10000)
    encumbered_no_label.scroll_into_view_if_needed()
    pause(page, 500)

    try:
        encumbered_no_label.click()
    except Exception:
        encumbered_no_label.click(force=True)

    log_step("Encumbered set to No")
    pause(page, 800)
    # encumbered ends here
    fill_input_with_fallback(
        page,
        [
...
def fill_crop_details_form(page, survey_number: str) -> None:
    ...
    crop_name = "Bulbous Flowers | Summer Crop (March-June)"
    select_dropdown_value(
        page,
        [
            page.locator("#cropId"),
